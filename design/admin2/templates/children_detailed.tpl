...
{def $extraListColumnInformation = ext_getExtraColumnInfo()}
...
{/switch}
    ...
    dataSourceURL: "{concat('ezjscore/call/bfchildlimitation::subTree::', $node.node_id)|ezurl('no')}",
...
var labelsObj = {ldelim}
    ...
    DATA_TABLE_COLS: {ldelim}
						...
						// insert stuff programmatically, based on needed items (see README-CUSTOMATTRIBUTECOLUMNS.txt)
						{foreach $extraListColumnInformation as $columnId => $columnHash}
							{$columnId}: "{$columnHash.Label|i18n( 'design/admin/node/view/full' )|wash('javascript')}",
						{/foreach}
                        // keep that one last, so we don't have to worry about last comma
...
{if and( $node.is_container,  $node.can_create)}
    ...
    {def $can_create_classes = fetch( 'content', 'can_instantiate_class_list', hash( 'parent_node', $node,
                                                                                     'filter_type', 'exclude',
                                                                                     'group_id', $group_id,
                                                                                     'group_by_class_group', true() ) ) | ext_bfchildlimitation_recomputeAllowedChildren($node)}
...
YUILoader.onSuccess = function() {
	...
	{foreach $extraListColumnInformation as $columnId => $columnHash}
		extraColumnDefs.push(
			{ldelim}
			key:"{$columnId}",
			label: "{$columnHash.Label|i18n( 'design/admin/node/view/full' )|wash('javascript')}",
			sortable: {if $columnHash.IsSortable}true{else}false{/if},
			resizeable: {if $columnHash.IsResizable}true{else}false{/if}
			{rdelim});
	{/foreach}
	{literal}
	sortableSubitems.init(confObj, labelsObj, createGroups, createOptions, extraColumnDefs);
};
var options = [];
