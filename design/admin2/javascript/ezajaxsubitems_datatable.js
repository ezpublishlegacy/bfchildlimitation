var sortableSubitems = function () {
    ...
    function initDataTable(extraColumnDefs){
        ...
        for (var i=0; i<extraColumnDefs.length; i++) {
        	columnDefs[columnDefs.length] = extraColumnDefs[i];
        }
        ...
        // insert custom fields into keys
        for (var i=0; i<extraColumnDefs.length; i++) {
        	dataSource.responseSchema["fields"].push({key: extraColumnDefs[i].key});
        }
    ...
    return {
        init: function (conf, labels, groups, options, extraColumnDefs) {
            confObj = conf
            labelsObj = labels;
            createGroups = groups;
            ...
            dataTable = initDataTable(extraColumnDefs);
        }
    };
